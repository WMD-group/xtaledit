...
and the same multiset of Wyckoff letters (regardless of which elements occupy
each letter).  The set of equivalent letter assignments for a structure is
generated by applying the coset representatives of the **Euclidean normalizer**
of the space group.  Pymatgen ships this data in
...
def match_wyckoff(
    ...
    # Index: (spg, letter_key) -> list of (j, k2) — struct2 index + relabeling index.
    train_index: dict[tuple[int, tuple[str, ...]], list[tuple[int, int]]] = {}
    for j, data2 in enumerate(data2_list):
        for k2, lkey in enumerate(data2.letter_key):
            train_index.setdefault((data2.spg_num, lkey), []).append((j, k2))
    ...
    for i, data1 in tqdm(
        ...
        # best[j] = (min_cost, repr1_idx, repr2_idx, orbit_map)
        best: dict[int, tuple[float, int, int, list[int]]] = {}
        ...
        for k1, lkey in enumerate(data1.letter_key):
            ...
            for j, k2 in train_index.get(idx_key, []):
                data2 = data2_list[j]
                ...
                if j not in best or cost_val < best[j][0]:
                    best[j] = (cost_val, r1, r2, orbit_map)

        for j, (cost_val, repr1_idx, repr2_idx, orbit_map) in best.items():
            data2 = data2_list[j]
